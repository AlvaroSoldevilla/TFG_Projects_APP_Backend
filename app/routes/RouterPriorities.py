...
async def get_all_priorities(session: Session = Depends(get_session)):
    return await sp.get_all_priorities(session)


@router.get("/{id}", response_model=PriorityRead, status_code=200)
async def get_priority_by_id(id: int, session: Session = Depends(get_session)):
    return await sp.get_priority_by_id(id, session)


@router.post("/", status_code=200)
async def create_priority(priority_data: PriorityCreate, session: Session = Depends(get_session)):
    if await sp.create_priority(priority_data, session):
...
async def update_priority(id: int, priority_update: PriorityUpdate, session: Session = Depends(get_session)):
    if await sp.update_priority(id, priority_update, session):
...
async def delete_priority(id: int, session: Session = Depends(get_session)):
    if await sp.delete_priority(id, session):
