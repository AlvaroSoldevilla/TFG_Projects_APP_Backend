...
async def get_all_concept_boards(session: Session = Depends(get_session)):
    return await scb.get_all_concept_boards(session)


@router.get("/{id}", response_model=ConceptBoardRead, status_code=200)
async def get_concept_board_by_id(id: int, session: Session = Depends(get_session)):
    return await scb.get_concept_board_by_id(id, session)


@router.post("/", status_code=200)
async def create_concept_board(concept_board_data: ConceptBoardCreate, session: Session = Depends(get_session)):
    if await scb.create_concept_board(concept_board_data, session):
...
async def update_concept_board(id: int, concept_board_update: ConceptBoardUpdate, session: Session = Depends(get_session)):
    if await scb.update_concept_board(id, concept_board_update, session):
...
async def delete_concept_board(id: int, session: Session = Depends(get_session)):
    if await scb.delete_concept_board(id, session):
...
async def get_concept_boards_by_concept(id_concept: int, session: Session = Depends(get_session)):
    return await scb.get_concept_boards_by_concept(id_concept, session)
