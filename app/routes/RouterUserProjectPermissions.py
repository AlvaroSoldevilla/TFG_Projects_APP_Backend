...
# Generic endpoints
@router.get("", dependencies=[Depends(jwt_scheme)], response_model=list[UserProjectPermissionRead], status_code=200)
...
@router.get("/{id}", dependencies=[Depends(jwt_scheme)], response_model=UserProjectPermissionRead, status_code=200)
def get_user_project_permission_by_id(id: int, session: Session = Depends(get_session)):
    user_project_permission = supp.get_user_project_permission_by_id(id, session)
    if user_project_permission is None:
        raise HTTPException(status_code=404, detail="UserProjectPermission not found")
    else:
        return user_project_permission


@router.post("", dependencies=[Depends(jwt_scheme)], status_code=200, response_model=UserProjectPermissionRead)
...
@router.get("/user/{id}", dependencies=[Depends(jwt_scheme)], response_model=list[UserProjectPermissionRead], status_code=200)
...
@router.get("/project/{id}", dependencies=[Depends(jwt_scheme)], response_model=list[UserProjectPermissionRead], status_code=200)
...
@router.get("/permission/{id}", dependencies=[Depends(jwt_scheme)], response_model=list[UserProjectPermissionRead], status_code=200)
...
@router.get("/user/{id_user}/project/{id_project}", dependencies=[Depends(jwt_scheme)], response_model=list[UserProjectPermissionRead], status_code=200)
