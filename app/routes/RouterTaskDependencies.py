...
async def get_all_task_dependencies(session: Session = Depends(get_session)):
    return await std.get_all_task_dependencies(session)


@router.get("/{id}", response_model=TaskDependencyRead, status_code=200)
async def get_task_dependency_by_id(id: int, session: Session = Depends(get_session)):
    return await std.get_task_dependency_by_id(id, session)


@router.post("/", status_code=200)
async def create_task_dependency(task_dependency_data: TaskDependencyCreate, session: Session = Depends(get_session)):
    if await std.create_task_dependency(task_dependency_data, session):
...
async def update_task_dependency(id: int, task_dependency_update: TaskDependencyUpdate, session: Session = Depends(get_session)):
    if await std.update_task_dependency(id, task_dependency_update, session):
...
async def delete_task_dependency(id: int, session: Session = Depends(get_session)):
    if await std.delete_task_dependency(id, session):
...
async def get_task_dependencies_by_task(task_id: int, session: Session = Depends(get_session)):
    return await std.get_dependencies_by_task_id(task_id, session)
