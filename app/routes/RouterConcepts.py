...
async def get_all_concepts(session: Session = Depends(get_session)):
    return await sc.get_all_concepts(session)


@router.get("/{id}", response_model=ConceptRead, status_code=200)
async def get_concept_by_id(id: int, session: Session = Depends(get_session)):
    return await sc.get_concept_by_id(id, session)


@router.post("/", status_code=200)
async def create_concept(concept_data: ConceptCreate, session: Session = Depends(get_session)):
    if await sc.create_concept(concept_data, session):
...
async def update_concept(id: int, concept_update: ConceptUpdate, session: Session = Depends(get_session)):
    if await sc.update_concept(id, concept_update, session):
...
async def delete_concept(id: int, session: Session = Depends(get_session)):
    if await sc.delete_concept(id, session):
...
async def get_concepts_by_project(id_project: int, session: Session = Depends(get_session)):
    return await sc.get_concept_boards_by_project(id_project, session)
